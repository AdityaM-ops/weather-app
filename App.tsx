...
const App: React.FC = () => {
  ...
  const getLatestData = useCallback((lat?: number, lon?: number, label?: string) => {
    setLoading(true);
    setError(null);
    setNextRefresh(5);

    if (lat !== undefined && lon !== undefined) {
      fetchLiveWeather(lat, lon, label)
        .then(setLiveData)
        .catch(() => setError("Failed to fetch weather data for the searched location"))
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setError("Location permission denied. Showing default data.");
        setLoading(false);
      },
      { timeout: 10000 }
    );
  ...
  const display = liveData || {
    ...
    locationLabel: 'Default (Mock Data)',
    timestamp: 'Initial sync',
    ...
    if (config.type === 'thermometer') {
      return (
        <svg width="24" height="60" viewBox="0 0 24 60" className="shrink-0 overflow-visible ml-auto z-10">
          ...
          <rect x="9" y={45 - (percent * 35)} width="6" height={percent * 35 + 5} rx="3" fill={colorCode} className="transition-all duration-1000 ease-out" />
        ...
        return (
          <div className="space-y-8 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="relative bg-white p-10 rounded-[2.5rem] shadow-sm border border-[#AAB2C0]/10 flex flex-col items-center text-center group overflow-hidden">
                <WeatherBgEffect condition={display.condition} />
                <h3 className="relative z-10 text-[11px] font-black text-[#AAB2C0] mb-8 tracking-[0.2em] uppercase">Visual Classification</h3>
                <div className="relative z-10 mb-8 transform transition-transform group-hover:scale-110 duration-500">
                  <div className="absolute inset-0 bg-[#4A90E2]/20 blur-[50px] opacity-40"></div>
                  <div className="relative drop-shadow-2xl">
                    {React.cloneElement(CONDITION_ICONS[display.condition as keyof typeof CONDITION_ICONS] || CONDITION_ICONS.Sunny, { className: "w-20 h-20" })}
                  </div>
                </div>
              ...
              <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-[#AAB2C0]/10 space-y-6">
                ...
                <div className="p-6 bg-[#F5F7FA] rounded-3xl border border-[#AAB2C0]/10">
                  <p className="text-[10px] text-[#AAB2C0] uppercase font-black tracking-widest mb-2">Location Index</p>
                  <p className="text-xl font-black text-[#1F2A44]">{display.locationLabel}</p>
                </div>
                <div className="p-6 bg-[#F5F7FA] rounded-3xl border border-[#AAB2C0]/10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-[#AAB2C0]/10 space-y-8">
                ...
                <div className="space-y-6">
                   <div className="flex items-center justify-between p-6 bg-[#F5F7FA] rounded-3xl">
                      <div>
